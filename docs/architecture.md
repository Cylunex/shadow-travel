...
PostgreSQL 是生产数据库。首个迁移先建立身份、OIDC 流程、会话、主题地图、成员、地点、来源、点位、到访、媒体引用、路线、幂等记录和审计表。所有主键使用 UUID；时间使用 UTC；共享对象默认软删除。

迁移只通过 Alembic 前向执行。上线前必须在同版本数据库副本完成升级、启动和回滚演练；应用回滚采用整版回滚，不跨版本混用数据库模型。

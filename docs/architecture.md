...
Provider 地点唯一键包含地点所有者，避免不同用户导入同一平台地点时复用私有 Place。共享记录与审计时间线使用稳定的复合排序键和不透明游标，数据库为常用主题时间线建立复合索引；照片数量通过分组子查询批量聚合，不按记录逐条查询。公开分享的 `last_accessed_at` 最多每 15 分钟写入一次，避免读取流量转化成同等规模的数据库更新。

CSV/GeoJSON 预览属于有明确资源边界的同步工作：正文最多 5 MB，单次最多解析 1000 个点位，并逐行收集错误。未来若需要放宽上限，必须改成可持久化、可重试、可查询状态的后台导入任务，由独立 Worker 执行；不使用进程内临时后台任务承载大文件。
